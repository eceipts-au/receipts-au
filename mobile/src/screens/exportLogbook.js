...
export async function exportCSV({ vehicleId = null, periodId = null } = {}) {
  ...
  // Header required by ATO-style recordkeeping (friendly version)
  const header = [
    "Vehicle Rego",
  ...
  for (const t of trips) {
    rows.push([
      "", // you can fill rego if you store it on vehicle
      t.usage,
      t.purpose || "",
      t.startTimeISO,
      t.endTimeISO || "",
      ...
      t.kms ?? "",
      t.startLoc?.place ||
        (t.startLoc ? `${t.startLoc.lat},${t.startLoc.lng}` : ""),
  ...
  }logbook_export_${Date.now()}.csv`;
  await FileSystem.writeAsStringAsync(pathname, csv, {
